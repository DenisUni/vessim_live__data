...
def convert_log_level(string):
    ...
    if string.lower() in log_levels:
        ...
        pre_logger.error(f"valid is: DEBUG, INFO, WARNING, ERROR, CRITICAL")
        raise KeyError


def setup_logger(name: str, display_name: str = None) -> logging.Logger:
...
log_level_file = convert_log_level(settings.LOG_LEVEL_FILE)
log_level_console = convert_log_level(settings.LOG_LEVEL_CONSOLE)
