...
class WattTimeService:
    ...
    def __init__(self, username: Optional[str] = None, password: Optional[str] = None):
        ...
        self.token = None
        self.logger = setup_logger(__name__, "WATTTIME-SERVICE")

        if self.username and self.password:
            self.login()
        else:
            self.logger.warning("WattTime credentials not provided. API calls requiring auth will fail.")
    ...
    def register(self, username: str, password: str, email: str, *, org: Optional[str] = None) -> bool:
        ...
        if response.ok:
            ...
            self.logger.error(
                f"WattTime registration failed: {response.status_code} {responses[response.status_code]} {response.json()}")
            return False
    ...
    def get_historical_data(self, region: str, start_time: datetime, end_time: datetime,
                            signal_type: str = 'co2_moer') -> pd.DataFrame:
        """
