...
logger = setup_logger(__name__, "WATTTIME-API")
...
def startup():
    ...
    """
    logger.info("Checking WattTime API connection...")

    if not watttime_service.check_connection():
        logger.warning(
            "WattTime connection failed. Starting interactive setup... (maybe press enter twice for continuing)")
        _interactive_setup()
    else:
        logger.info("WattTime connection verified.")
...
def _interactive_setup():
    ...
    while True:
        choice = input(
            "Do you want to (L)ogin with existing credentials or (R)egister a new account? [L/R]: ").strip().upper()
...
def get_overview():
    ...
    return {
        ...
        "endpoints": [
            {"path": "/carbon/", "method": "GET",
             "description": "Retrieve WattTime carbon intensity data for a given Region (region) and time range. Fetches from cache or WattTime API."},
            {"path": "/carbon/", "method": "POST",
             "description": "Manually store a single WattTime carbon data point in the cache."},
            {"path": "/carbon/fetch-range/", "method": "POST",
             "description": "Trigger a background task to fetch and store a range of WattTime carbon data."},
            {"path": "/region-from-loc/", "method": "GET",
             "description": "Get the Region (Balancing Authority) for a given latitude and longitude."},
...
def fetch_range_task(region: str, start_time: datetime, end_time: datetime, session: Session):
    ...
    try:
        ...
        if df.empty:
            logger.warning(f"No data found for {region} from {start_time} to {end_time}")
            return
        ...
        session.commit()
        logger.info(f"Successfully fetched and stored carbon data for {region} from {start_time} to {end_time}")
    except Exception as e:
